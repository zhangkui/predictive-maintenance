...
func (ThresholdDetector) Detect(current Sample, history []Sample, c Config) Result {
	...
	if current.Value < c.MinValue || current.Value > c.MaxValue {
		r.Abnormal = true
		...
		distance := 0.0
		if current.Value < c.MinValue {
			distance = c.MinValue - current.Value
		} else {
...
func (ThresholdDetector) NormalizedDistance(current Sample, c Config) float64 {
	if current.Value < c.MinValue {
		return (c.MinValue - current.Value) / (c.MaxValue - c.MinValue)
	}
